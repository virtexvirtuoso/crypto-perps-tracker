...
<body>
    <div class="header">
        <h1>₿ CRYPTO SPAGHETTI CHART - {hours}H</h1>
        <div class="bybit-badge">🏦 Data Source: Bybit Exchange</div>
        <div class="subtitle">Actual Price Returns - All Symbols Normalized</div>
    ...
    <div class="legend-info">
        ...
        <p>
            Each line shows <strong>actual price returns</strong> over {hours} hours. All symbols start at 0% (normalized from their starting price).
            This "spaghetti chart" style lets you instantly see which coins had the strongest price movements and compare relative performance.
            Lines are color-coded by their Bitcoin Beta (correlation strength).
        </p>
        <p style="margin-top: 10px;">
            <strong>Example:</strong> If BTC line shows +2%, it gained 2% over the period. If ETH shows +3%, it gained 3% over the same period.
            The chart shows raw performance - not relative to BTC.
        </p>
        <p style="margin-top: 10px;">
            ...
            Hover over any line to see details including volume rank. Top 10 performers shown by default - click legend to show more.
        </p>
        <p style="margin-top: 10px;">
            <strong>Reference Lines:</strong> Dotted lines at ±5% and dashed lines at ±10% help identify significant price moves.
        </p>
    ...
    <script>
        ...
        // Get beta values for coloring
        var symbolAnalysis = {json.dumps({a['symbol']: a.get('btc_beta', 1.0) for a in symbols_with_beta})};

        // Calculate final returns for each symbol for sorting
        var symbolReturns = {{}};
        symbolsInChart.forEach(function(symbol) {{
            let candles = historicalData[symbol];
            if (candles && candles.length > 0) {{
                let initialPrice = candles[0].close;
                let finalPrice = candles[candles.length - 1].close;
                let returnPct = ((finalPrice - initialPrice) / initialPrice) * 100;
                symbolReturns[symbol] = returnPct;
            }} else {{
                symbolReturns[symbol] = 0;
            }}
        }});

        // Sort symbols by actual return (highest to lowest)
        symbolsInChart.sort(function(a, b) {{
            return symbolReturns[b] - symbolReturns[a];  // Descending by return
        }});
        ...
        symbolsInChart.forEach(function(symbol, index) {{
            ...
            // Calculate actual price returns (spaghetti chart style)
            // All symbols start at 0% and show their actual price movement
            let initialPrice = candles[0].close;
            let percentChanges = candles.map(c => ((c.close - initialPrice) / initialPrice) * 100);

            // Get final return for legend
            let finalReturn = percentChanges[percentChanges.length - 1];
            let returnStr = (finalReturn >= 0 ? '+' : '') + finalReturn.toFixed(2) + '%';
            ...
            // Show only top 10 symbols by default (by performance)
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 Generating Crypto Spaghetti Chart ({HOURS}H - Bybit Only)...\n")
    print("⏳ Fetching data from Bybit...\n")
    ...
    filename = os.path.join(data_dir, f"crypto_spaghetti_chart_{HOURS}h_bybit_{timestamp}.html")

    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"✅ Crypto Spaghetti Chart ({HOURS}H) saved to: {filename}")
        print(f"\n🌐 Open in browser to view the {HOURS}-hour actual returns chart!")
