...
<body>
    <div class="header">
        <h1>₿ BITCOIN BETA CORRELATION - {hours}H</h1>
        <div class="bybit-badge">🏦 Data Source: Bybit Exchange</div>
        <div class="subtitle">Theoretical Beta Correlation with Bitcoin</div>
    ...
    <div class="legend-info">
        ...
        <p>
            Each line shows the <strong>theoretical beta correlation</strong> with Bitcoin over {hours} hours. BTC is shown as a flat line at 0% (baseline).
            All other symbols display their expected movement based on their beta: if BTC moves +1% and a symbol has 17x beta, it shows +17%.
        ...
        <p style="margin-top: 10px;">
            <strong>Reference Lines:</strong> Dotted lines at ±5% and dashed lines at ±10% help identify correlation strength.
        </p>
    ...
    <script>
        ...
        // Get BTC price changes first (for theoretical correlation calculation)
        ...
        symbolsInChart.forEach(function(symbol, index) {{
            ...
            // Calculate theoretical beta correlation
            // BTC = 0%, all others = beta * BTC's movement
            let percentChanges;
            if (symbol === 'BTC') {{
                // BTC is the baseline at 0%
                percentChanges = btcPercentChanges.map(() => 0);
            }} else {{
                // Theoretical movement: if BTC moves +1% and beta is 17.72x, show +17.72%
                // If beta is -10x and BTC moves +1%, show -10%
                percentChanges = btcPercentChanges.map(btcChange => btcChange * beta);
            }}
        ...
        var chartLayout = {{
            ...
            yaxis: {{
                title: 'Theoretical Beta Correlation (%)',
                gridcolor: 'rgba(255, 215, 0, 0.1)',
                color: '#FFD700',
                zeroline: true,
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 Generating Bitcoin Beta Time Series ({HOURS}H - Bybit Only)...\n")
    print("⏳ Fetching data from Bybit...\n")
    ...
    filename = os.path.join(data_dir, f"bitcoin_beta_timeseries_{HOURS}h_bybit_{timestamp}.html")

    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"✅ Bitcoin Beta Time Series ({HOURS}H) saved to: {filename}")
        print(f"\n🌐 Open in browser to view the {HOURS}-hour movement chart!")
